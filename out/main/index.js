...
const electronApp = {
  setAppUserModelId(id2) {
    if (platform.isWindows)
      electron.app.setAppUserModelId(is.dev ? process.execPath : id2);
  },
  setAutoLaunch(auto) {
    if (platform.isLinux)
      return false;
...
async function execCommand(command, args, root, options) {
  const cliPath = await findCli();
...
async function execRaw(command, args, root, options) {
  const cliPath = await findCli();
...
const schema = {
  collections: {
    ...
    items: {
      ...
      required: ["id", "name", "path", "addedAt", "lastOpenedAt"]
    }
  },
  activeCollectionId: {
    type: ["string", "null"],
...
function addCollection(path$12) {
  const s = initStore();
  const collections = s.get("collections", []);
  const duplicate = collections.find((c) => c.path === path$12);
  if (duplicate) {
    throw new Error(`Collection already exists for path: ${path$12}`);
  }
  const now = Date.now();
  const collection = {
    id: crypto.randomUUID(),
    name: path.basename(path$12),
    path: path$12,
    addedAt: now,
    lastOpenedAt: now
  };
...
function removeCollection(id2) {
  const s = initStore();
  const collections = s.get("collections", []);
  s.set(
    "collections",
    collections.filter((c) => c.id !== id2)
  );
  if (s.get("activeCollectionId") === id2) {
    s.set("activeCollectionId", null);
  }
}
function setActiveCollection(id2) {
  const s = initStore();
  const collections = s.get("collections", []);
  const index = collections.findIndex((c) => c.id === id2);
  if (index === -1) {
    throw new Error(`Collection not found: ${id2}`);
  }
  collections[index] = { ...collections[index], lastOpenedAt: Date.now() };
  s.set("collections", collections);
  s.set("activeCollectionId", id2);
}
...
async function validateCollectionPath(path$12) {
  const name = path.basename(path$12) || path$12;
  if (!path$12) {
    return { valid: false, hasConfig: false, name: "", error: "Path is empty" };
  }
  if (!fs.existsSync(path$12)) {
    return { valid: false, hasConfig: false, name, error: "Path does not exist" };
  }
  try {
    const stat = fs.statSync(path$12);
    if (!stat.isDirectory()) {
  ...
  const hasConfig = fs.existsSync(`${path$12}/.markdownvdb`) || fs.existsSync(`${path$12}/.markdownvdb/.config`);
...
async function initCollection(path2) {
  await execRaw("init", [], path2);
}
async function confirmRemoveCollection(name) {
...
async function promptInitCollection(name) {
  const result = await electron.dialog.showMessageBox({
    type: "question",
...
const INGEST_TIMEOUT_MS = 3e5;
function serializeError(error) {
  if (error instanceof CliNotFoundError || error instanceof CliExecutionError || error instanceof CliParseError || error instanceof CliTimeoutError) {
    return error.serialize();
  }
  if (error instanceof Error) {
    return { error: true, type: "CliExecutionError", message: error.message };
  }
  return { error: true, type: "CliExecutionError", message: String(error) };
}
...
function registerIpcHandlers() {
  electron.ipcMain.handle("cli:find", () => wrapHandler(() => findCli()));
  electron.ipcMain.handle("cli:version", () => wrapHandler(() => getCliVersion()));
  electron.ipcMain.handle(
    "cli:search",
    (_event, root, query, options) => {
      const args = [query];
      ...
      return wrapHandler(() => execCommand("search", args, root));
    }
  );
  electron.ipcMain.handle(
    "cli:status",
    (_event, root) => wrapHandler(() => execCommand("status", [], root))
  );
  electron.ipcMain.handle(
    "cli:ingest",
    (_event, root, options) => {
      ...
      return wrapHandler(
        () => execCommand("ingest", args, root, { timeout: INGEST_TIMEOUT_MS })
      );
    }
  );
  electron.ipcMain.handle(
    "cli:ingest-preview",
    (_event, root) => wrapHandler(
      () => execCommand("ingest", ["--preview"], root)
    )
  );
  electron.ipcMain.handle("cli:tree", (_event, root, path2) => {
    const args = [];
    if (path2) args.push("--path", path2);
    return wrapHandler(() => execCommand("tree", args, root));
  });
  electron.ipcMain.handle(
    "cli:get",
    (_event, root, filePath) => wrapHandler(() => execCommand("get", [filePath], root))
  );
  electron.ipcMain.handle(
    "cli:links",
    (_event, root, filePath) => wrapHandler(() => execCommand("links", [filePath], root))
  );
  electron.ipcMain.handle(
    "cli:backlinks",
    (_event, root, filePath) => wrapHandler(() => execCommand("backlinks", [filePath], root))
  );
  electron.ipcMain.handle(
    "cli:orphans",
    (_event, root) => wrapHandler(() => execCommand("orphans", [], root))
  );
  electron.ipcMain.handle(
    "cli:clusters",
    (_event, root) => wrapHandler(() => execCommand("clusters", [], root))
  );
  electron.ipcMain.handle(
    "cli:schema",
    (_event, root) => wrapHandler(() => execCommand("schema", [], root))
  );
  electron.ipcMain.handle(
    "cli:config",
    (_event, root) => wrapHandler(() => execCommand("config", [], root))
  );
  electron.ipcMain.handle(
    "cli:doctor",
    (_event, root) => wrapHandler(() => execCommand("doctor", [], root))
  );
  electron.ipcMain.handle(
    "cli:init",
    (_event, root) => wrapHandler(() => execRaw("init", [], root))
  );
  electron.ipcMain.handle(
    "collections:list",
  ...
  electron.ipcMain.handle(
    "collections:add",
    () => wrapHandler(async () => {
      const path2 = await pickCollectionFolder();
      if (!path2) return null;
      const validation2 = await validateCollectionPath(path2);
      if (!validation2.valid) {
        throw new Error(validation2.error ?? "Invalid collection path");
      }
      if (!validation2.hasConfig) {
        const shouldInit = await promptInitCollection(validation2.name);
        if (!shouldInit) return null;
        await initCollection(path2);
      }
      return addCollection(path2);
    })
  );
  electron.ipcMain.handle(
    "collections:remove",
    (_event, id2) => wrapHandler(async () => {
      const collections = getCollections();
      const collection = collections.find((c) => c.id === id2);
      if (!collection) {
        throw new Error(`Collection not found: ${id2}`);
      }
      const confirmed = await confirmRemoveCollection(collection.name);
      if (!confirmed) return;
      removeCollection(id2);
    })
  );
  electron.ipcMain.handle(
    "collections:set-active",
    (_event, id2) => wrapHandler(async () => {
      setActiveCollection(id2);
    })
  );
  electron.ipcMain.handle(
    "collections:get-active",
    () => wrapHandler(async () => getActiveCollection())
  );
  electron.ipcMain.handle(
    "fs:read-file",
    (_event, absolutePath) => wrapHandler(async () => {
      const { resolve: resolve2, sep } = await import("node:path");
      const normalizedPath = resolve2(absolutePath);
      const collections = getCollections();
