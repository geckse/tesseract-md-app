...
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `cli:`         | find, version, search, status, ingest, ingest-preview, tree, get, links, backlinks, orphans, clusters, schema, collection, config, doctor, init | mdvdb CLI commands (`cli:get` and `cli:collection` accept `{ populate?: boolean }` → `--populate`, phase 42) |
