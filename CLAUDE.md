...
- **Collection agent skills (phase 46)**: `src/main/collection-skills.ts`, spec in `docs/prds/phase-46-collection-agent-skills.md`. The app bundles only `../tesseract-skills/plugins/tesseract` and checks active collections for a current copy in `.claude/skills`, `.agents/skills`, or `.gemini/skills`. A content+version fingerprint detects updates; `CollectionSkillsNotification.svelte` offers an active-collection footer reminder, while Collection Settings always exposes every target and supports multi-target install/update. Session dismissal and per-collection permanent dismissal apply only to the reminder. Main-process path validation and symlink rejection protect collection boundaries.
