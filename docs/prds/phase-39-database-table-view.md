...
- **Integration** (`vitest.integration.config.mts`): IPC registration + error serialization for `cli:collection` (correct `--sort`/`--order` split + repeatable `--filter` arg construction), `fs:update-frontmatter` (real `yaml` against a temp file, body preservation + `file:saved-externally` broadcast to a mocked second window), `tableviews:*`.
