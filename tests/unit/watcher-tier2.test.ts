...
function tree(): FileTree {
  return {
    root: {
      ...
      state: null,
      children: [
        { name: 'note.md', path: 'note.md', is_dir: false, state: 'modified', children: [] }
      ]
    },
    total_files: 1,
    indexed_count: 0,
...
describe('Tier-2 watch-event integration (mdvdb reindex → app patch)', () => {
  it('flips the file tree node to indexed on a successful reindex', () => {
    handleWatcherEvent({
      type: 'watch-event',
      data: report({ event_type: 'Modified', path: 'note.md', success: true })
    })
  ...
  it('optimistically bumps document_count on a successful Created event', () => {
    ...
    handleWatcherEvent({
      type: 'watch-event',
      data: report({ event_type: 'Created', path: 'created.md', success: true })
    })
  ...
  it('does NOT touch the tree or graph when the reindex failed', async () => {
    handleWatcherEvent({
      type: 'watch-event',
      data: report({ success: false, error: 'api error' })
    })
    const node = get(fileTree)!.root.children.find((c) => c.path === 'note.md')
  ...
  it('handles the CLI startup line harmlessly', () => {
    ...
    expect(() =>
      handleWatcherEvent({
        type: 'watch-event',
        data: { status: 'watching', message: 'x' } as unknown as WatchEventReport
      })
    ).not.toThrow()
