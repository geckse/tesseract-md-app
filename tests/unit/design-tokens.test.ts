...
const tokensCSS = readFileSync(resolve(__dirname, '../../src/renderer/styles/tokens.css'), 'utf-8')
...
describe('Design Tokens — tokens.css', () => {
  ...
  describe('Shadows', () => {
    it('defines elevation shadows for popovers, modals, and the sticky table column', () => {
      expect(tokens.get('--shadow-popover')).toBe('0 8px 24px rgba(0, 0, 0, 0.45)')
      expect(tokens.get('--shadow-modal')).toBe('0 12px 40px rgba(0, 0, 0, 0.5)')
      expect(tokens.get('--shadow-sticky-col')).toBe('2px 0 8px rgba(0, 0, 0, 0.35)')
    })
  })
